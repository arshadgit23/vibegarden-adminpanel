...
const FeaturedTools = () => {
  ...
  useEffect(() => {
    const getToolVideos = async () => {
      try {
        setLoading(true);
        const response = await apicall.get(`/videos/featured?videoType=tool`, {
          params: { search: filterTerm },
        });
        setToolVideos(response.data.data);
        setLoading(false);
      } catch (error) {
        setError(
  ...
  const insertVideoHandler = async (e) => {
    try {
      setLoading(true);
      const response = await apicall.patch("/videos/featured/tools", {
        featuredToolIds: videosId,
      });
  ...
  const handleVideoDetail = (id) => {
    navigate(`/single-featured-tool-video/${id}`);
  };
    ...
    <>
      ...
      <div className="bg-[white] shadow-lg shadow-[#00000029]-500/50 w-[95%] h-[600px] rounded-[20px] m-10 overflow-y-auto">
        ...
        {loading ? (
          ...
          <h1 className="ml-10">No Featured Tool Found</h1>
        ) : (
          <div class="md:grid grid-cols-4 gap-2 ">
            {toolVideos?.map((val, ind) => (
              <div
                key={ind}
          ...
          <div class="md:grid grid-cols-4 gap-2 overflow-y-auto">
            {videos?.map((val, ind) => (
              <div
                ...
                className={`bg-[white] shadow-lg shadow-[#00000029]-500/50 cursor-pointer w-[200px] h-[300px] rounded-[20px] m-5 ${
