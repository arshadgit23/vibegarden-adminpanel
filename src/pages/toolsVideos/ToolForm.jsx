...
const ToolForm = ({
  ...
  //   queryKey: ["getAllTags"],
  //   queryFn: async () => {
  //     return await apicall.get(`/tags?type=tool`);
  //   },
  // });
  ...
  useEffect(() => {
    const getAllTags = async () => {
      setTagsLoading(true);
      try {
        const response = await apicall.get(`/tags?type=tool`);
        setAllTags(response);
  ...
  const updateToolVideo = async (e) => {
    ...
    updatedToolVideo.append("videoType", "tool");
    ...
    await updateFunction(updatedToolVideo);
    setTimeout(() => {
      navigate("/ToolsVideos");
      setSubmitLoading(false);
    }, 3000);
...
export default React.memo(ToolForm);
